...
class Solution
{
    ...
    public static boolean Ancestors(Node root , ArrayList<Integer> path , int target){
        ...
        path.add(root.data);
        if(Ancestors(root.left , path , target )== true )return true ;
        if(Ancestors(root.right , path , target)== true ) return true ;
    ...
    {
        // add your code here
        ArrayList<Integer> ansestors = new ArrayList<>();
        Ancestors(root , ansestors , target);
        Collections.reverse(ansestors);
        return ansestors ;
    }
}
